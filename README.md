# Object Oriented Programming Patterns and Problems

This repository contains notes and examples of various design patterns as well as common OOP interview problems implemented in Python.

## Table of Contents

- [Behavioral Design Patterns](design_patterns/behavioral/behavioral_design_patterns.md)
    - [Chain of Responsibility](design_patterns/behavioral/behavioral_design_patterns.md#chain-of-responsibility)
    - [Command](design_patterns/behavioral/behavioral_design_patterns.md#command)
    - [Observer](design_patterns/behavioral/behavioral_design_patterns.md#observer)
    - [Strategy](design_patterns/behavioral/behavioral_design_patterns.md#strategy)
- [Creational Design Patterns](design_patterns/creational/creational_design_patterns.md)
  - [Abstract Factory](design_patterns/creational/creational_design_patterns.md#abstract-factory-pattern)
  - [Factory Method](design_patterns/creational/creational_design_patterns.md#factory-method)
  - [Builder](design_patterns/creational/creational_design_patterns.md#builder)
  - [Prototype](design_patterns/creational/creational_design_patterns.md#prototype)
  - [Singleton](design_patterns/creational/creational_design_patterns.md#singleton)
- [Structural Design Patterns](design_patterns/structural/structural_design_patterns.md)
  - [Adapter](design_patterns/structural/structural_design_patterns.md#adapter)
  - [Bridge](design_patterns/structural/structural_design_patterns.md#bridge)
  - [Composite](design_patterns/structural/structural_design_patterns.md#composite)
  - [Decorator](design_patterns/structural/structural_design_patterns.md#decorator)
  - [Facade](design_patterns/structural/structural_design_patterns.md#facade)
  - [Flyweight](design_patterns/structural/structural_design_patterns.md#flyweight)
  - [Proxy](design_patterns/structural/structural_design_patterns.md#proxy)
- [SOLID Principles](#solid-principles)
- [Common OOP Interview Problems](#common-oop-interview-problems)
  - [Call Center](#call-center)
  - [Deck of Cards](#deck-of-cards)
  - [Dictionary](#dictionary)
  - [LRU Cache](#lru-cache)
  - [Online Chat](#online-chat)
  - [Parking Lot](#parking-lot)

## Design Patterns

### Behavioral

- [Behavioral Design Patterns](design_patterns/behavioral/behavioral_design_patterns.md)
    - [Chain of Responsibility](design_patterns/behavioral/chain_of_responsibility.py)
    - [Command](design_patterns/behavioral/command.py)
    - [Observer](design_patterns/behavioral/observer.py)
    - [Strategy](design_patterns/behavioral/strategy.py)

### Creational

- [Creational Design Patterns](design_patterns/creational/creational_design_patterns.md)
    - [Abstract Factory](design_patterns/creational/abstract_factory.py)
    - [Builder](design_patterns/creational/builder.py)
    - [Factory Method](design_patterns/creational/factory_method.py)
    - [Prototype](design_patterns/creational/prototype.py)
    - [Singleton](design_patterns/creational/singleton.py)

### Structural

- [Structural Design Patterns](design_patterns/structural/structural_design_patterns.md)
    - [Adapter](design_patterns/structural/adapter.py)
    - [Bridge](design_patterns/structural/bridge.py)
    - [Composite](design_patterns/structural/composite.py)
    - [Decorator (Functional)](design_patterns/structural/decorator_func.py)
    - [Decorator (OOP)](design_patterns/structural/decorator_oop.py)
    - [Facade](design_patterns/structural/facade.py)
    - [Flyweight](design_patterns/structural/flyweight.py)
    - [Proxy](design_patterns/structural/proxy.py)

## SOLID Principles

- [SOLID Principles](solid_principles.md)

## Common OOP Interview Problems

### Call Center

- [Call Center](problems/call_center/call_center.py)
- [Employee](problems/call_center/employee.py)
- [Levels](problems/call_center/levels.py)
- [Request](problems/call_center/request.py)

### Deck of Cards

- [Card](problems/deck_of_cards/card.py)
- [Deck](problems/deck_of_cards/deck.py)
- [Game](problems/deck_of_cards/game.py)
- [Suit](problems/deck_of_cards/suit.py)

### Dictionary

- [Dictionary](problems/dictionary.py)

### LRU Cache

- [LRU Cache](problems/lru_cache.py)

### Online Chat

- [Online Chat](problems/online_chat/online_chat.py)

### Parking Lot

- [Parking Lot](problems/parking_lot/parking_lot.py)

